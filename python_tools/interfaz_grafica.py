...
def procesar_videos():
    ...
    def worker():
        try:
            ...
            video_pred = os.path.join(OUTPUT_DIR, "EdgeImpulse9_linea_tiempo_monitoreo.mp4")
            video_accel = os.path.join(OUTPUT_DIR, "EdgeImpulse9_acelerometro_monitoreo.mp4")
...
root = tk.Tk()
root.title("Reconstrucción Visual — Edge Impulse 9 Clases")
...
tk.Label(main_frame, text="Herramienta de Videos — Edge Impulse (9 Clases)", font=font_title, bg="#f4f4f9", fg="#333").pack(pady=(0, 20))

instrucciones = (
    "Sigue estos pasos:\n\n"
    "1. Ve a la carpeta que se acaba de crear llamada 'input_json'.\n"
    "2. Coloca ahi el archivo JSON exportado de la aplicación (solo 1).\n"
    "   Nombre esperado: datos-monitoreo-EdgeImpulse9-clases.json\n"
